...
pub struct UniversalMmapChunk<T: Copy + Sized + 'static> {
    ...
    cached: Option<crate::cache::CachedFile>,
}
...
pub fn read_mmaps<T: Copy + Sized>(
    ...
    for chunk_id in 0..num_chunks {
        ...
        let cached = crate::cache::CacheController::global()
...
fn get_cached<'a, T: Copy + Sized + 'static>(
    file: &'a crate::cache::CachedFile,
    range: Range<usize>,
) -> MmapChunkView<'a, T> {
    let slice = file.get_range(range).into_owned();
    MmapChunkView::Box(
        unsafe { crate::cache::unsafe_transmute_zerocopy_vec(slice) }.into_boxed_slice(),
    )
}
